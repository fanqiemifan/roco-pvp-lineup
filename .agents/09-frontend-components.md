...
- components/ — 可复用小组件（Page4SlotVisual、SettingField、SpritePetCard、StageThumb）
...
- roster - 阵容编辑（RosterPanelEditor）
  - 左右面板编辑、精灵搜索、快速填充；顶部「当前比赛」表单含左右选手名 + 排位排名（仅数字，PATCH 保存比赛信息时一并提交）。
...
- stage - 直播推流（卡片式设置）
  ...
  - 画面切换行为（过渡效果 none/blinds/zoom；page10 胜者结算自动切入与停留时长 page10Duration）。
